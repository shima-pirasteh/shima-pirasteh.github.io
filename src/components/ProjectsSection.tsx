...
export default function ProjectsSection({ onSelectProject }: ProjectsSectionProps) {
  ...
  const filteredProjects = useMemo(() => {
    return PROJECTS.filter((v) => {
      ...
      const matchesTools = selectedTools.length === 0 || selectedTools.every((tool) => v.tools.includes(tool));
    ...
    <section id="projects" className="pt-32 pb-24 bg-[#0A0A0B] px-6 md:px-12 lg:px-24 min-h-screen">
      <div className="max-w-6xl mx-auto">
        ...
        <div id="projects-desc" className="mb-16">
          ...
          <h2 className="font-serif text-3xl md:text-4xl text-stone-100 tracking-tight leading-none">Portfolio</h2>
          <p className="mt-4 text-stone-400 text-sm md:text-base max-w-2xl">
            A comprehensive overview of architectural models, computational skills, and interactive narrative spaces.
            Filter below by work dimension or production tool.
          </p>
        </div>
